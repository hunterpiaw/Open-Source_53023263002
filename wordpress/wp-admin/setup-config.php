...
if ( ! file_exists( ABSPATH . 'wp-config-sample.php' ) )
	wp_die( __( 'ขอโทษ  แต่ระบบต้องการไฟล์ wp-config-sample.php เพื่อทำงาน กรุณาอัปโหลดไฟล์นี้อีกครั้งจากการติดตั้งเวิร์ดเพรสของคุณ' ) );

$config_file = file(ABSPATH . 'wp-config-sample.php');

// Check if wp-config.php has been created
if ( file_exists( ABSPATH . 'wp-config.php' ) )
	wp_die( '<p>' . sprintf( __( "มีไฟล์ 'wp-config.php' อยู่แล้ว ถ้าคุณต้องการตั้งค่าใหม่ในไฟล์นี้   กรุณาลบมันก่อน  คุณอาจจะลอง <a href='%s'>ติดตั้งตอนนี้</a>" ), 'install.php' ) . '</p>' );

// Check if wp-config.php exists above the root directory but is not part of another install
if ( file_exists(ABSPATH . '../wp-config.php' ) && ! file_exists( ABSPATH . '../wp-settings.php' ) )
	wp_die( '<p>' . sprintf( __( "มีไฟล์ 'wp-config.php' อยู่แล้วเหนือการติดตั้งเวิร์ดเพรสของคุณขั้นหนึ่ง ถ้าคุณต้องการตั้งค่าใหม่ในไฟล์นี้   กรุณาลบมันก่อน  คุณอาจจะลอง <a href='install.php'>ติดตั้งตอนนี้</a>"), 'install.php' ) . '</p>' );
...
<p class="step"><a href="setup-config.php?step=1<?php if ( isset( $_GET['noapi'] ) ) echo '&amp;noapi'; ?>" class="button button-large"><?php _e( 'เริ่มการติดตั้ง' ); ?></a></p>
<?php
	break;
...
<form method="post" action="setup-config.php?step=2">
	...
	<table class="form-table">
		...
		<tr>
			...
			<td><?php _e( '&hellip;และรหัสผ่าน MySQL' ); ?></td>
		</tr>
		<tr>
